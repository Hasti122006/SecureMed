-- Role checks use user_has_app_role() only — never EXISTS (SELECT … FROM user_roles)
-- inside policies ON user_roles (that pattern causes infinite RLS recursion).
-- Superseded in spirit by 20260323120000_fix_infinite_rls_recursion.sql; kept for
-- migration history. Re-run 20260323120000 if you still see recursion from an old DB.
